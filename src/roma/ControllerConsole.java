...
public class ControllerConsole implements IController {
	...
	public Card getCard(List<Card> cardList, String message) {
		...
		cardIndex = getInt("[Enter 0-" + (cardList.size() - 1) + "]: ");
	...
	public void showScreen(Player p) {
		...
		showMessage("It is player " + (g.getCurrentPlayer().getPlayerId() + 1)
				+ "'s turn.");
		showMessage("------------------------ FIELD --------------------");
		showField();
		...
		System.out.println("You have " + VP + " Victory Points and " + money
				+ " Sesterii");
		showMessage("---------------------------");
		System.out.println("Discard pile: "
				+ g.getDiscardPile().asList().size());
