...
public class ActivateCardAction implements IPlayerAction {
	...
	public void query(GameVisor g) {
		...
		targetPos = g.getController().getInt(
		...
		if (targetPos >= 1 && targetPos <= Game.FIELD_SIZE) {
			...
			if (targetPos == Game.BRIBE_DISC) {
				bribeDice = g
						.getController()
						.getInt("Choose the Dice you want to use to activate the bribe disc.");
				confirmBribe = g
						.getController()
						.getBoolean(
								"Are you sure you want to activate that disc? Doing so will cost extra Sestertii.");
			} else {
				confirmBribe = true;
			}
